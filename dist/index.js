...
log.request = debug('axios:request');
log.response = debug('axios:response');
export default class FedExAPI {
  ...
  createShipment(params) {
    return this.request.post('/v1/shipments', params);
  }
  /**
   *
  ...
  estimate(params) {
    return this.request.post('/v1/estimates', params);
  }
  /**
   *
  ...
  updateShipment(id, params) {
    return this.request.post(`/v1/shipments/${id}`, params);
  }
